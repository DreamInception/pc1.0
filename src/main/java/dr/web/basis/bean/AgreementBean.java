/**
 *	包名:	dr.web.basis.controller
 *	文件名:	Bean.java
 *	描述:	登录相关操作
 *	创建人:	陈吉秋特
 *	创建时间:	2016-3-16下午5:00:00
 *	版权:	2016 景源金服版权所有
 */
package dr.web.basis.bean;
import dr.web.common.bean.BaseBean;
/**
 * 
 * @包名 :dr.web.basis.controller
 * @文件名 :AgreementBean.java
 * @系统名称 : 上海景源金服PC端
 * @Author: 陈吉秋特
 * @Date: 2014-6-4 上午9:56:20
 * @版本号 :v0.0.01
 */
public class AgreementBean  extends BaseBean {

	/**agreement_id*/public static final String agreementId = "agreementId" ;                                           /**agreement_name*/public static final String agreementName = "agreementName" ;                                           /**agreement_introduction*/public static final String agreementIntroduction = "agreementIntroduction" ;                                           /**agreement_content*/public static final String agreementContent = "agreementContent" ;                                           /**agreement_createtime*/public static final String agreementCreatetime = "agreementCreatetime" ;                                           
}
