...
<head lang="en">
    ...
    <script type="text/javascript" src="${base}/static/safeCenter/js/resetTransactPsw.js"></script>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $("#rlp-pwd").on("input", function (event) {
                ...
                validatePassword(element,"rlp-setPsw-error","密码为6位数字");
            });
            $("#rlp-confirmPsw").on("input", function (event) {
                var element = event.target;
                validateSecondPsw(element,tranPsw,"rlp-doublePsw-error","两次输入的交易密码不符");
            });
...
<article id="main" class="mid-Container f-index-dline f-cb">
    ...
    <section class="user-detail f-fl">
        ...
        <p class="rlp-title">修改交易密码</p>
        <div class="rtp-resetTPswDiv">
        <input type="text" value="${userTelephone}" id="rlp-phoneNum" class="rlp-phoneNum" placeholder="手机号" disabled="disable">
        <p id="rlp-phoneNum-error" class="errorMsg"></p>
        <div class="f-cb">
        <input id="resetTransact" type="text" class="rlp-verifyCode f-fl" placeholder="验证码">
        <img id="resetTransactImg" src="${base}/authImage/image" class="rlp-codeStyle f-fr" onclick="refreshRand('resetTransactImg')">
    	</div>
    	<p id="rtp-picCode-error" class="errorMsg"></p>    
        <div class="f-cb">
        	<input type="hidden" id="ticket">
        	<input type="hidden" id="tpl" value="resetTradePwd">
            <input type="text" id="texting" class="rlp-msgCode f-fl" placeholder="短信验证码">
            <input type="button" id="rlp-gainCode" class="rlp-gainPsw f-fr" value="获取验证码" onclick="sendMessage('rlp-phoneNum','rlp-gainCode','rlp-msgCode-error','手机号不能为空','tpl','resetTransact')"/>
      
        </div>
		<p id="rlp-msgCode-error" class="errorMsg"></p>
        <input type="password" id="rlp-pwd" class="rlp-password" placeholder="请输入6位数字密码">
        ...
        <a href="javascript:resetTransact()" class="rlp-submit f-fl">确定</a>
        </div>
        </article>
    </section>
</article>
